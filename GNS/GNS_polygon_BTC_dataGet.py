...
r = requests.get('https://api.dune.com/api/v1/query/2825251/results/csv?api_key=H9rqlaFKqoEQqA4DIHiTgmuUbDOpgWYP', params=params)
r = r.content
GNS_transaction_1 = pd.read_csv(io.StringIO(r.decode('utf-8')))

# the 2nd part
params = {
  "format": "csv"
}
r = requests.get('https://api.dune.com/api/v1/query/2829105/results/csv?api_key=H9rqlaFKqoEQqA4DIHiTgmuUbDOpgWYP', params=params)
r = r.content
GNS_transaction_2 = pd.read_csv(io.StringIO(r.decode('utf-8')))

# merge and processing
GNS_transaction=pd.concat([GNS_transaction_1,GNS_transaction_2])
...
############################### merge
GNS_liquidation_data=pd.merge(GNS_liquidation_daily,GNS_tradingVolume, how="left", on="time")
GNS_liquidation_data=pd.merge(GNS_liquidation_data, GNS_openInterest, how="left", on="time")
GNS_liquidation_data=pd.merge(GNS_liquidation_data, GNS_leverage_long, how="left", on="time")
GNS_liquidation_data=pd.merge(GNS_liquidation_data, GNS_leverage_short, how="left", on="time")
GNS_liquidation_data=pd.merge(GNS_liquidation_data, GNS_leverage_category, how="left", on="time")
GNS_liquidation_data=pd.merge(GNS_liquidation_data, BTC_OHLC, how="left", on="time")

############################### trucate the data
GNS_liquidation_Leverage = GNS_liquidation_data.loc[GNS_liquidation_data['time']<='2023-09-26',:]
GNS_liquidation_Leverage=GNS_liquidation_Leverage.fillna(0)
GNS_liquidation_Leverage.to_csv('/Users/andyma/Desktop/Python /Perpetual_futures/GNS/GNS_polygon_liquidation.csv')